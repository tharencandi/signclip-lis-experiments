...
from signclip.tasks.signit_prolip_finetune import fineTuneSignITProLIP


def main():
    parser = argparse.ArgumentParser(description="SignIT ProLIP Fine-tuning Launcher")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/signclip_v1_1/signit_prolip_finetune.yaml",
    ...
    save_dir = config.fairseq.checkpoint.get("save_dir", "runs/signclip_signit_prolip_finetune")
    os.makedirs(save_dir, exist_ok=True)

    finetuner = fineTuneSignITProLIP(config, checkpoint_path=checkpoint_path)
