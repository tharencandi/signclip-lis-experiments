...
from signclip.tasks.a3lis_prolip_finetune import fineTuneA3LISProLIP


def main():
    parser = argparse.ArgumentParser(description="A3LIS ProLIP Fine-tuning Launcher")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/signclip_v1_1/a3lis_prolip_finetune.yaml",
    ...
    save_dir = config.fairseq.checkpoint.get("save_dir", "runs/signclip_a3lis_prolip_finetune")
    os.makedirs(save_dir, exist_ok=True)

    finetuner = fineTuneA3LISProLIP(config, checkpoint_path=checkpoint_path)
    ...
    if args.jobtype == "local_single":
        ...
        for epoch in range(num_epochs):
            ...
            if val_score > best_score:
                ...
                print(
                    f"  Saved best checkpoint (score: {val_score:.4f} - "
                    f"R@1: {val_r1:.2f}% R@5: {val_r5:.2f}% medK: {val_medK:.1f} at epoch {epoch+1})"
                )
